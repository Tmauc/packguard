...
| **8b — Publishing + Nalo validation** | `cargo publish` + Homebrew tap + Docker push + step PackGuard dans vrai `.gitlab-ci.yml` Nalo avec preuve MR bloquée | 🟡 partiel (2026-04-22) : GitHub Release v0.1.0 ✅ + Docker Hub `mauc/packguard` ✅ + ghcr.io `tmauc/packguard` ✅ + cosign ✅ + **Homebrew tap `Tmauc/homebrew-packguard`** ✅ + **crates-publish workflow + version-sanity + bump-version.sh** ✅. Reste : premier `cargo publish` (manuel via `gh workflow run crates-publish.yml`), Nalo GitLab CI, automation Homebrew (besoin `HOMEBREW_TAP_TOKEN`) |
