...
A fresh store (no scan yet) shows a call-to-action pointing to [`packguard scan`](/cli/scan). The dashboard doesn't try to auto-trigger a scan — scans are expensive and should happen at a time the user chose.
