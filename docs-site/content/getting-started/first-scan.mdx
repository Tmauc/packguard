...
**`init`** drops a `.packguard.yml` with `offset: -1`, `stability: stable`, `min_age_days: 7`, and blocking on high/critical CVEs + malware + deprecated + yanked. Typosquat defaults to `warn` (noisy signal, human-in-the-loop review).
