...
- **Debug** (`cargo run -p packguard-cli -- ui`) only serves `/api/*`. Run `pnpm --dir dashboard dev` alongside it so Vite on `:5173` proxies `/api/*` for hot-reload during development.
